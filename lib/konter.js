...
konter.cron = cron;
konter.plugins = {};
konter.jadeCompile = jadeCompile;
konter.root = path.normalize( __dirname + '/../' );
konter.env = process.env.NODE_ENV;
konter.dashboard = { plugins: []};
 ...
 *
 * @param {Object}
 * * envOnly: (default: false) do not start up the server, just initialize the
 *   environment
 *
 * @api public
 */
konter.server.start = function startServer( options ){

  options = options || { envOnly: false };
...
function loadPlugins(){
  ...
  fs.readdirSync( pluginsDir ).forEach( function( pluginFileName ){
    konter.plugins[ pluginFileName.replace('.js','') ] = require( path.join( pluginsDir, pluginFileName ) );
    konter.logger.info('adding plugin', pluginFileName.replace('.js',''));
  });

}

function jadeCompile( filename, locals ){
  locals = locals || {};
  return jade.compile( fs.readFileSync( filename ), { filename: filename } )( locals );
}