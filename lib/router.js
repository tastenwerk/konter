...
router.use = function( filename ){
  ...
  try{
    ...
    if( !fs.existsSync( filename ) )
      filename = path.join( process.cwd(), config.appsDir, filename, filename+'_routes' );

    if( fs.existsSync( filename+'.js' ) )
      require( filename )( app, socket );

    var viewsPath = path.join( path.dirname(filename), 'views' );

    if( fs.existsSync( viewsPath ) )
      views.add( viewsPath );

    logger.info( path.basename(filename.replace(path.extname(filename),'')), 'initialized.');
